...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, File.* from Sample, File
           where (Sample.idData = 20) AND (File.idSample = Sample.id AND File.type like "%CpG%")')
dfSample = dbGetQuery(db, q)
nrow(dfSample)
dfSample
# remove any whitespace from the names
...
oGRLmerged = GRangesList(gr1, gr2, gr3, gr4, gr5, gr6)
rm(list=c('gr1', 'gr2', 'gr3', 'gr4', 'gr5', 'gr6'))
gc(reset = T)
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/',
                comment='GRangesList object for CpG methylation from S135 run for bs seq data for miho ishida produced by the methylation extractor script from bismark')
