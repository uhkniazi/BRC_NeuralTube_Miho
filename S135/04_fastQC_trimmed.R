...
setwd('dataExternal/raw/S135/Trimmed/')
...
setwd(gcswd)
n = make.names(paste('CFastqQuality trimmed S135 run for Miho BS Seq project id 8'))
lOb$meta.1 = dfSample
lOb$meta.2 = dfFiles
lOb$desc = paste('CFastqQuality object trimmed S135 run for Miho BS Seq project id 8', date())
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='S135 run for Miho BS Seq project trimmed FASTQ file quality data')
...
cvSampleNames = gsub('trim_(M\\d+).+', '\\1', dfFiles$name)
...
mAlphabets = sapply(lAlphabets, function(x) return(x[,'T']))
