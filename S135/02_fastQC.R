...
g_did
dfSample = dbGetQuery(db, "select * from Sample where idData=20;")
...
setwd('dataExternal/raw/Raw/S135/S135/processed/')
...
setwd(gcswd)
n = make.names(paste('CFastqQuality S135 run for Miho BS Seq project id 8'))
lOb$meta.1 = dfSample
lOb$meta.2 = dfFiles
lOb$desc = paste('CFastqQuality object S135 run for Miho BS Seq project id 8', date())
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='S135 run for Miho BS Seq project FASTQ file quality data')
...
barplot(iReadCount, las=2, main='Pre-Trim Read Count', ylab = 'No. of Reads in Millions', cex.names =0.8, col=grey.colors(2), ylim=c(0, 250))
...
dbListTables(db)
g_did
q = paste0('select File.id as fid, File.name, File.type, Sample.* from File, Sample where Sample.idData = 20 AND (File.idSample = Sample.id AND File.type like "%fastq%")')
