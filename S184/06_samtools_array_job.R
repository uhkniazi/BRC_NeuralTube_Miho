...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 27) and (File.idSample = Sample.id) and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 27 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "fastq")')
...
# dfNewData$group1 = 'Generated from Trimmomatic standard input for bisulphite seq data S184'
