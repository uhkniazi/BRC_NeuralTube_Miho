...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, File.* from Sample, File
           where (Sample.idData = 27) AND (File.idSample = Sample.id AND File.type like "%CpG%")')
dfSample = dbGetQuery(db, q)
nrow(dfSample)
dfSample
# remove any whitespace from the names
...
oGRLmerged = GRangesList(gr1, gr2)
rm(list=c('gr1', 'gr2'))
gc(reset = T)
...
setwd(gcswd)
n = make.names(paste('GRangesList object for CpG methylation from bs seq S135 miho rds'))
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/',
                comment='GRangesList object for CpG methylation from S184 run for bs seq data for miho ishida produced by the methylation extractor script from bismark')
