...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, File.* from Sample, File
           where (Sample.idData = 27) AND (File.idSample = Sample.id AND File.type like "%duplicates removed%")')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold rd picard bs seq S184 miho rds'))
lAllBams$metaBismark = dfBismark
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from S184 run for bs seq data for miho ishida with quality 10 duplicates removed using picard tools', date())
...
#                 comment='CBamScaffold object from S184 run for bs seq data for miho ishida with quality 10 duplicates removed and standard settings on trimmomatic
...
write.csv(dfBismark, file='results/bismarkReport.csv')
