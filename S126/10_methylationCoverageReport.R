...
# get the query
g_did
q = paste0('select * from MetaFile where (idData = 15) AND (comment like "%methylation%")')
...
#lCoverage = list()
lCoverage$"624" = mCoverage
## repeat until all 8 samples done


# comment out as this has been done once
n = make.names(paste('List for CpG methylation coverage from bs seq S126 miho rds'))
...
#                 comment='List object for CpG methylation coverage from S126 run for bs seq data for miho ishida produced by the methylation extractor script from bismark')
...
# get the query
g_did
q = paste0('select * from Sample where (idData = 15)')
dfSample = dbGetQuery(db, q)
nrow(dfSample)
dfSample
# close connection after getting data
